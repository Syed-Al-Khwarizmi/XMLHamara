...
namespace File6Namespace
{
    class File6XmltoXml
    {
        ...
        private static XElement individualRowCreator(List<string> fields)
        {
            ...
            string[] fieldNamesOther = { "DATAID", "VERSIONNUM", "GENDER", "SUBMITTED_BY", "COUNTRY", "TITLE", "DESIGNATION" };
            ...
            for (int a = 0; a < fieldNamesOther.Length; a++)
            {
                if (fields.Contains(fieldNamesOther[a]))
                {
                    ...
                    {
                        int k = 1;
                        if (fieldNamesOther[a] == "TITLE" || fieldNamesOther[a] == "DESIGNATION")
                            k = 2;
        ...
        public static void File6(string filename, string output, string datetime)
        {
